...
export default function ApartmentCard({ apartment, priority = false }: ApartmentCardProps) {
  ...
  return (
    <Link
      ...
      className="group block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-heart-sage-100"
    >
      {/* Image */}
      ...
      {/* Content */}
      <div className="p-5">
        <h3 className="text-lg font-bold text-heart-charcoal-800 mb-1 group-hover:text-heart-coral-500 transition-colors font-heading">
          {apartment.name}
        </h3>
        <p className="text-sm text-heart-charcoal-600 mb-3 line-clamp-1">
          {data.subtitle}
        </p>

        {/* Airbnb-style Specs */}
        <div className="text-sm text-heart-charcoal-600 mb-4">
        ...
        <div className="flex items-center justify-between">
          <span className="text-heart-coral-500 font-medium text-sm group-hover:underline">
            {t('viewDetails')} →
          </span>
        </div>
      </div>
